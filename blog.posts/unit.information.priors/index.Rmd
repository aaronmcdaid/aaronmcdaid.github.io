...
and use that to decide whether to select $H_1$ or $H_{\hat\tau^{-1}}$.
...
$$
\mathrm{\hat{P}}(\mathbf{x} | H_{\hat\tau^{-1}}) > \mathrm{     P }(\mathbf{x} | H_1)
$$
...
We can then compare $\mathrm{P}_\tau(\mathbf{x})$ to $\mathrm{P}(\mathbf{x}|H_1)$ as usual.

In practice, we compute that integral in the code below via the following equation, with $\tau$ on the right hand side replaced by an arbitrary value:
$$
\mathrm{P}(\mathbf{x})
=
\frac{
...
The effect of the $n$ observations on $\alpha$ is to add $\frac{n}2$.
...
We could define the *zero information prior* as $\mathrm{Gamma}(0,0)$ (useless as a prior, as it is [*improper*](https://en.wikipedia.org/wiki/Prior_probability#Improper_priors)).
...
Note that prior does depend on the observed data. However, it is a relatively diffuse prior as it's based on the average effect of one observation instead of the combined effect of $n$ observations.
...
1. $\mathrm{P}(\mathbf{x} | H_1)$ - assuming Hypothesis 1 is true, i.e. $\tau = 1   = \sigma^2$
1. $\mathrm{P}(\mathbf{x} | H_2)$ - assuming Hypothesis 1 is true, i.e. $\tau = 0.5;  \sigma^2 = 2$
1. $\mathrm{P}(\mathbf{x} | H_{mle})$ - assuming the true variance equals the MLE estimate in the sample.
1. $\mathrm{P}(\mathbf{x} | H_{uip})$ - log-likelihood, after interesting over our unit information prior.
1. $\mathrm{P}(\mathbf{x} | H_{exact})$ - log-likelihood, after interesting over the "true" prior, $\mathrm{Gamma}(1,1)$
...
First, we define the function `a.few.loglikelihoods` which takes a sample as input
and computes those five log-likelihoods and returns them.

I make heavy use of the `data.table` package in `R`, by the way.
...
Now that the data has been prepared above, we close a few plots to show which model is selected.
The final figure confirms that the integrated log-loglikelihood based on the unit information
prior, $H_{uip}$, doesn't have the extreme bias of using the MLE, $H_{mle}$.
We know that $H_{exact}$ is the best possible in this case, as it uses the true distribution
of $\tau$ and therefore the Neyman-Pearson Lemma applies directly.
In the last two plots, we can see that $H_{exact}$ and $H_{uip}$ get very similar results,
confirming that $H_{uip}$ maintains good accuracy despite not knowing the true prior for $\tau$,
which was $\mathrm{Gamma}(1,1)$.

### Figure 1

In Figure 1, we see that
the log-ratio is always positive comparing $H_{mle}$ to $H_1$, which confirms that $H_{mle}$ is not useful.

```{r fig.align='center'}
d[tau >= 0.1 & tau <= 10.0,{
    plot(   tau ,   P_x_Hmle - P_x_H1
        ,   xlab = expression(tau) ,   ylab = expression('log-ratio of ' * H[mle] * ' and ' * H[1])
        ,   log='x',   main = 'Figure 1'
        )
    abline(h=0, col='grey')
}] %>% invisible
...
d[tau >= 0.25 & tau < 2.0,{
    plot(   tau,   P_x_H2-P_x_H1
        ,   xlab = expression(tau) ,   ylab = expression('log-ratio of ' * H[2] * ' and ' * H[1])
        ,   log='x',   main='Figure 2'
        )
    abline(h=0, col='grey')
    abline(v=1, col='grey')
...
d[
  ...
  ,{
    plot(   c(1,NA,tau)
        ,   c(NA,0,P_x_Hexact - P_x_H1)
        ,   xlab = expression(tau)
        ,   ylab = expression('log-ratio of ' * H[exact] * ' and ' * H[1])
        ,   log='x', main='Figure 3'
        )
    abline(h=0, col='grey')
    abline(v=1, col='grey')
...
d[
  tau >= 0.5 & tau < 2.0
  ,{
    plot(   tau ,   P_x_Huip - P_x_H1
        ,   xlab = expression(tau)
        ,   ylab = expression('log-ratio of ' * H[uip] * ' and ' * H[1])
        ,   log='x', main = 'Figure 4'
        )
    abline(h=0, col='grey')
    abline(v=1, col='grey')
...
different from 1.0. Other, more conventional, methods exist for this.
...
Very complex models can be used in this framework, as long as you have distributions with
