...
a.few.loglikelihoods <- function(x) {
    ...
    #stopifnot(mle_var / 2 == 1/(2*n) * sum(x^2) ) # kinda obvious, just making sure I rearranged them correctly
...
d[tau >= 0.1 & tau <= 10.0,{
    plot(   c(1,NA,tau)
        ,   c(NA,0,P_x_Hmle - P_x_H1)
        ,   log='x'
        ,   xlab = expression(tau)
        ,   ylab = expression('log-ratio of ' * H[mle] * ' and ' * H[1])
        #,   ylim = c(0,100)
        ,   sub = expression('The log-ratio is always positive comparing ' * H[mle] * 'to ' * 'H_1' * ', confirming H_mle is not useful')
        )
    abline(h=0, col='grey')
}] %>% invisible
d[tau >= 0.25 & tau < 2.0,{
    plot(   c(1,NA,tau)
        ,   c(NA,0,P_x_H2 - P_x_H1)
        ,   log='x'
        ,   xlab = expression(tau)
        ,   ylab = expression('log-ratio of ' * H[2] * ' and ' * H[1])
        #,   ylim = c(0,100)
        #,   xlim= c(0.5, 4)
        ,   sub = expression('The two vertical grey lines at 0.5 and 1.0 represent when '*H[2]*' and '*H[1]*', respectively, are true. No surprise the log-ratio crosses 0 in between those points')
        )
    abline(h=0, col='grey')
    abline(v=1, col='grey')
...
d[
  ...
  ,{
    plot(   c(1,NA,tau)
        ...
        #,   ylim = c(0,100)
        #,   xlim= c(0.5, 4)
        ,   sub = expression('Using the "exact" prior, which is cheating. As expected, '*H[1]*' is selected (y-axis < 0) when '*tau*' is close to '
                             *1.0*' and the rejected (y-axis > 0) when '*tau*' is far from '*1.0)
        )
    abline(h=0, col='grey')
    abline(v=1, col='grey')
}] %>% invisible
d[
  ...
  ,{
    plot(   c(1,NA,tau)
        ,   c(NA,0,P_x_Huip - P_x_H1)
        ,   log='x'
        ,   xlab = expression(tau)
        ,   ylab = expression('log-ratio of ' * H[uip] * ' and ' * H[1])
        #,   ylim = c(0,100)
        #,   xlim= c(0.5, 4)
        ,   sub = expression('Using our UIP prior. As expected, '*H[1]*' is selected (y-axis < 0) when '*tau*' is close to '
                             *1.0*' and the rejected (y-axis > 0) when '*tau*' is far from '*1.0)
        )
    abline(h=0, col='grey')
    abline(v=1, col='grey')
